  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-extrabold tracking-tight text-slate-900">{t('my_listed_properties')}</h1>
          <p className="text-sm text-slate-500">{t('manage_listings_note')}</p>
        </div>
        <Link
        ...
        >
          <PlusCircle className="h-4 w-4" /> {t('post_property')}
        </Link>
      </div>
      ...
      {properties.length === 0 ? (
        <div className="flex flex-col items-center justify-center gap-3 rounded-3xl border border-slate-200 bg-white py-16 text-slate-400 shadow-sm">
          ...
          <h2 className="text-lg font-bold text-slate-700">{t('no_properties_yet')}</h2>
          <p className="text-sm max-w-xs text-center">{t('no_properties_note')}</p>
          {user.status === 'Approved' && (
              ...
              <div key={p.id.toString()} className="group overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm transition hover:shadow-md">
                ...
                <div className="p-5">
                  ...
                  <p className="mt-1 text-lg font-extrabold text-orange-600">
                    {formatPrice(p.price)} ETB <span className="text-xs font-medium text-slate-400">{tv(p.priceType)}</span>
                  </p>
                  ...
                  <div className="mt-4 flex items-center justify-between border-t border-slate-100 pt-4">
                    <div className="flex gap-1">
                      <span className="rounded-md bg-slate-100 px-2 py-0.5 text-xs font-medium text-slate-500">{p.bedrooms} {t('beds')}</span>
                      <span className="rounded-md bg-slate-100 px-2 py-0.5 text-xs font-medium text-slate-500">{p.bathrooms} {t('baths')}</span>
