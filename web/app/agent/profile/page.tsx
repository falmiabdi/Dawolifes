        ...
        <div>
          ...
          {displayUser.status === 'Approved' && (
            <div className="flex items-center gap-2 rounded-xl bg-green-50 px-4 py-2 text-xs font-medium text-green-800 border border-green-100">
              <CheckCircle2 className="h-4 w-4" /> {t('verified_agent')}
            </div>
          )}
          {displayUser.status === 'Rejected' && (
            <div className="rounded-xl bg-red-50 px-4 py-2 text-xs font-medium text-red-800 border border-red-100 space-y-1">
              <div className="flex items-center gap-2">
                <XCircle className="h-4 w-4" /> {t('application_rejected')}
              </div>
              {displayUser.rejectionReason && <p className="text-slate-600">{t('reason_label')} {displayUser.rejectionReason}</p>}
              <Link
      ...
      <div className="grid gap-6 md:grid-cols-2">
        <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm space-y-4">
          ...
          <div className="divide-y divide-slate-100 text-sm">
            ...
            <div className="py-3 flex justify-between">
              ...
              <span className="font-semibold text-slate-800">{displayUser.dateOfBirth || t('not_specified')}</span>
            </div>
            <div className="py-3 flex justify-between">
            ...
            <div className="py-3 flex justify-between">
              <span className="text-slate-500">{t('preferred_language')}</span>
              <span className="font-semibold text-slate-800">{tv(displayUser.preferredLanguage || '') || t('not_specified')}</span>
            </div>
          </div>
        </div>
        ...
        <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm space-y-4">
          <div className="flex items-center gap-2 text-orange-600">
            ...
            <h2 className="font-bold text-slate-800">{t('contact_information')}</h2>
          </div>
          <div className="divide-y divide-slate-100 text-sm">
            <div className="py-3 flex justify-between">
              <span className="text-slate-500">{t('ethio_telecom_phone')}</span>
              <span className="font-semibold text-slate-800">{displayUser.ethPhone || t('not_specified')}</span>
            </div>
            <div className="py-3 flex justify-between">
              <span className="text-slate-500">{t('safaricom_phone')}</span>
              <span className="font-semibold text-slate-800">{displayUser.safaricomPhone || t('not_specified')}</span>
            </div>
            <div className="py-3 flex justify-between">
              <span className="text-slate-500">{t('region_city')}</span>
              <span className="font-semibold text-slate-800">
                {displayUser.region ? `${displayUser.region}, ${displayUser.city || ''}` : 'Not Specified'}
              </span>
            </div>
            <div className="py-3 flex justify-between">
              <span className="text-slate-500">{t('woreda_kebele')}</span>
              <span className="font-semibold text-slate-800">
        ...
        <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm space-y-4">
          <div className="flex items-center gap-2 text-orange-600">
            ...
            <h2 className="font-bold text-slate-800">{t('education_background')}</h2>
          </div>
          <div className="divide-y divide-slate-100 text-sm">
            <div className="py-3 flex justify-between">
              <span className="text-slate-500">{t('highest_education')}</span>
              <span className="font-semibold text-slate-800">{tv(displayUser.highestEducation || '') || t('not_specified')}</span>
            </div>
            <div className="py-3 flex justify-between">
              <span className="text-slate-500">{t('agent_experience')}</span>
              <span className="font-semibold text-slate-800">{tv(displayUser.agentExperience || '') || t('not_specified')}</span>
            </div>
            <div className="py-3 flex justify-between">
        ...
        <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm space-y-4">
          <div className="flex items-center gap-2 text-orange-600">
            ...
            <h2 className="font-bold text-slate-800">{t('uploaded_documents')}</h2>
          </div>
          <div className="grid grid-cols-2 gap-3">
            {[
              { label: t('doc_fayda_front'), url: displayUser.faydaFront },
              { label: t('doc_fayda_back'), url: displayUser.faydaBack },
              { label: t('doc_selfie'), url: displayUser.selfieFayda },
              { label: t('doc_passport'), url: displayUser.passportPhoto },
              { label: t('doc_education'), url: displayUser.educationCertificate },
              { label: t('doc_license'), url: displayUser.businessLicenseFile },
            ].map((doc) => (
