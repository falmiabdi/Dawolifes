...
import { MessageSquare, Send, User, Search, Phone, Mail } from 'lucide-react'
...
export default function AgentMessagesPage() {
  ...
  return (
    <div className="flex h-[calc(100vh-140px)] gap-6 overflow-hidden">
      {/* Threads list */}
      <div className="flex w-80 flex-col rounded-3xl border border-slate-200 bg-white shadow-sm overflow-hidden">
      ...
      {/* Main chat window */}
      {activeThread ? (
        <div className="flex-1 flex flex-col rounded-3xl border border-slate-200 bg-white shadow-sm overflow-hidden">
          {/* Header */}
          <div className="flex justify-between items-center px-6 py-4 border-b border-slate-100">
            <div>
              <h2 className="font-bold text-slate-950">{activeThread.name}</h2>
              <p className="text-xs text-slate-500 font-medium">Inquiry about: {activeThread.property}</p>
            </div>
            <div className="flex items-center gap-4 text-xs text-slate-500">
              <span className="flex items-center gap-1"><Phone className="h-3.5 w-3.5" /> {activeThread.phone}</span>
              <span className="flex items-center gap-1"><Mail className="h-3.5 w-3.5" /> {activeThread.email}</span>
            </div>
          </div>
            ...
            {activeThread.messages.map((m, idx) => {
              ...
              return (
                <div key={idx} className={`flex ${isAgent ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-md rounded-2xl px-4 py-2.5 text-sm ${isAgent ? 'bg-orange-500 text-white rounded-tr-none' : 'bg-white border border-slate-200 text-slate-800 rounded-tl-none'}`}>
                    <p>{m.text}</p>
