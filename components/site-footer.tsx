      ...
      <div className="mx-auto grid max-w-7xl grid-cols-1 gap-10 px-4 py-14 sm:px-6 md:grid-cols-2 lg:grid-cols-4">
        <div>
          ...
          <div className="mt-5 flex gap-3">
            {[Globe, Mail, Phone].map((Icon, i) => (
              <span
                key={i}
                className="flex h-9 w-9 items-center justify-center rounded-full bg-white/10 text-secondary-foreground/80"
              >
        ...
        <div>
          ...
          <ul className="mt-4 space-y-3 text-sm text-secondary-foreground/70">
            ...
            <li className="flex items-center gap-2">
              <MapPin className="h-4 w-4 text-primary" /> Addis Ababa, Ethiopia
            </li>
          </ul>
        </div>
      </div>
